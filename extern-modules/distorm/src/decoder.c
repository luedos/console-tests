...
size_t BinaryFindInt(unsigned int item, unsigned int const* arr, int size)
{
	if (!size || !arr) {
		return -1;
	}
...
size_t LiniarFindInt(unsigned int item, unsigned int const* arr, int size)
{
	if (!arr) {
		return - 1;
	}
...
int CheckForOrder(unsigned int const* arr, size_t size)
{
	if (!size || !arr) {
		return 0;
	}
...
{
	typedef size_t(*_SearchFunc)(unsigned int, unsigned int const*, int);

	_SearchFunc search = 0;

	if (CheckForOrder(_ci->relocs, _ci->relocSize)) {
		search = BinaryFindInt;
	}
	else {
		search = LiniarFindInt;
	}
	...
	*usedInstructionsCount = 0;
	...
	while (codeLen > 0) {
		...
		if (search(codeOffset, _ci->relocs, _ci->relocSize) != -1)
		{
			codeOffset += sizeof(unsigned int);
			code += sizeof(unsigned int);
			codeLen -= sizeof(unsigned int);
		}
